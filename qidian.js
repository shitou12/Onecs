...
🎯重写脚本:
[rewrite local]
https\:\/\/h5\.if\.qidian\.com\/argus\/api\/v1\/video\/adv\/finishWatch url script-request-body https://raw.githubusercontent.com/shitou12/Onecs/refs/heads/main/qidian.cookie.js
https\:\/\/h5\.if\.qidian\.com\/argus\/api\/v1\/video\/adv\/mainPage url script-response-body https://raw.githubusercontent.com/shitou12/Onecs/refs/heads/main/qidian.taskId.js
[MITM]
hostname = h5.if.qidian.com
⏰定时任务:
[task local]
30 10 * * * https://raw.githubusercontent.com/shitou12/Onecs/refs/heads/main/qidian.js, img-url=https://raw.githubusercontent.com/chxm1023/Script_X/main/icon/qidian.png, tag=起点读书, enabled=true
📦BoxJs地址:
...
$.timeout = $.getdata("qd_timeout") ? $.getdata("qd_timeout") : 0;

if (!$.taskId) {
  $.log("⚠️任务1信息不全!");
  $.log("请通过重写获取信息");
  $.msg($.name, "⚠️任务1信息不全!", "请通过重写获取信息");
  // $.done();
}
if (!$.taskId_2) {
  $.log("⚠️任务2信息不全!");
  $.log("请通过重写获取信息");
  $.msg($.name, "⚠️任务2信息不全!", "请通过重写获取信息");
  // $.done();
}
if (!$.session) {
  $.log("⚠️广告1信息不全!");
  $.log("请通过重写获取信息");
  $.msg($.name, "⚠️广告1信息不全!", "请通过重写获取信息");
  // $.done();
}
if (!$.session_2) {
  $.log("⚠️广告2信息不全!");
  $.log("请通过重写获取信息");
  $.msg($.name, "⚠️广告2信息不全!", "请通过重写获取信息");
  // $.done();
}
